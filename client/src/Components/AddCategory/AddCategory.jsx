...
function AddCategory({ addToList }) {
  ...
  const addNewCategory = () => {
    ...
    axios.post(`${process.env.REACT_APP_SERVER_API}/lists`, { name: inputValue }).then(({ data }) => {
      if (!inputValue) {
        return data
      }
  ...
  return (
    <>
      <div className='add-category' onClick={() => setVisiblePopup(!visiblePopup)}>
      ...
      {visiblePopup && (
        <>
          <div className='add-category__popup'>
            <img onClick={onClose} src={closePng} alt='Close popup' className='close-popup' />
            <input
              value={inputValue}
              onChange={event => {
