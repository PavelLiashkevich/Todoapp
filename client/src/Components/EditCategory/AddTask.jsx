...
function AddTask({ list, onAddTask }) {
  ...
  const addTask = () => {
    ...
    axios.post(`${process.env.REACT_APP_SERVER_API}/tasks`, obj).then(({ data }) => {
      if (!inputValue) {
    ...
    <div className='tasks__form'>
      {!visibleForm ? (
        ...
        <div className='tasks__form-block'>
          <div>
            <input className='field field-width' type='text' placeholder='Task' value={inputValue} onChange={event => setInputValue(event.target.value)} />
          </div>
